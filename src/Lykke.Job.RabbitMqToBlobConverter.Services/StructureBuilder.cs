...
namespace Lykke.Job.RabbitMqToBlobConverter.Services
{
    public class StructureBuilder : IStructureBuilder, ITypeInfo
    {
        ...
        public TablesStructure GetTablesStructure(Type type)
        {
            ...
            string parentIdPropertyTypeName)
        {
            ...
            {
                TableName = string.IsNullOrWhiteSpace(parentType?.Name)
                    ? (string.IsNullOrWhiteSpace(_instanceTag) ? typeName : $"{typeName}_{_instanceTag}")
                    : (string.IsNullOrWhiteSpace(_instanceTag) ? $"{parentType.Name}{typeName}" : $"{parentType.Name}{typeName}_{_instanceTag}"),
            ...
            if (collector.Columns.Count > 0)
            {
                if (parentIdPropertyName != null && parentType != type)
                {
                    ...
                    if (parentIdPropertyInChild == null)
                        collector.Columns.Insert(0, new ColumnInfo
                        {
